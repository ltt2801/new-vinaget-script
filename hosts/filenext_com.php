...
class dl_filenext_com extends Download {
	...
	public function CheckAcc($cookie){
        $data = $this->lib->curl("http://www.filenext.com/?op=my_account", "lang=english;{$cookie}", "");
        if(stristr($data, '<TD>Premium account expire</TD>')) return array(true, "Until ".$this->lib->cut_str($data, '<TD>Premium account expire</TD><TD><b>', '</b>'));
    ...
    public function Login($user, $pass){
		$data = $this->lib->curl("http://www.filenext.com/login.html", "lang=english", "op=login&login={$user}&password={$pass}&redirect=http://www.filenext.com/login.html");
