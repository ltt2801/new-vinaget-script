...
class dl_fireget_com extends Download
{
    ...
    public function CheckAcc($cookie)
    {
        ...
        if (stristr($data, 'Premium account expire')) {
            return array(true, "Until " . $this->lib->cut_str($data, 'Premium account expire:</b></TD><TD style="padding-left:10px;">', '</TD><TD>'));
    ...
    public function Leech($url)
    {
        ...
        if (stristr($data, 'You have reached the download-limit:')) {
            $this->error("LimitAcc", true, false);
        } elseif (stristr($data, 'type="password" name="password')) {
