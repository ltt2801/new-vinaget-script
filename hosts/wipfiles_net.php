...
class dl_wipfiles_net extends Download
{
    ...
    public function CheckAcc($cookie)
    {
        ...
        if (stristr($data, 'Premium account expire:')) {
            return array(true, "Until " . $this->lib->cut_str($data, '<TR><TD>Premium account expire:</TD><TD><b>', '</b>'));
        } else if (stristr($data, 'Payment info') && !stristr($data, 'Premium account expire:')) {
    ...
    public function Leech($url)
    {
        ...
        if ($pass) {
            ...
            } elseif (preg_match('@https?:\/\/(\w+\.)?wipfiles\.net(:\d+)?\/d\/[^"\'><\r\n\t]+@i', $data, $giay)) {
                return trim($giay[0]);
            }
        ...
        if (stristr($data, 'type="password" name="password')) {
            ...
            $post = $this->parseForm($this->lib->cut_str($data, '<Form name="F1"', '</Form>'));
            $data = $this->lib->curl($url, $this->lib->cookie, $post);
            if (preg_match('@https?:\/\/(\w+\.)?wipfiles\.net(:\d+)?\/d\/[^"\'><\r\n\t]+@i', $data, $giay)) {
                return trim($giay[0]);
            }
