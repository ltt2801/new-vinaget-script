...
class dl_filefactory_com extends Download
{
    ...
    public function Leech($url)
    {
        ...
        if (stristr($url, 'code=273')) {
            $this->error("You have exceeded the file or folder password attempt limit. Please try the download again later.", true, false);
        } elseif (stristr($url, 'code=251')) {
            $this->error("dead", true, false, 2);
        } elseif (stristr($url, 'code=253')) {
            $this->error("Server Maintenance", true, false);
        }
