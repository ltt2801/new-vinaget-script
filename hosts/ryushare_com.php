...
class dl_ryushare_com extends Download {
	...
	public function CheckAcc($cookie){
		...
		if(stristr($data, 'Premium account expire:')) return array(true, "Until ".$this->lib->cut_str($data, 'Premium account expire:</TD><TD><b>','</b></TD>'));
    ...
    public function Leech($url) {
		...
		if($pass) {
			...
			elseif(stristr($data,'>Error happened when generating Download Link.<'))   $this->error("Error happened when generating Download Link", true, false);
		...
		elseif(!$this->isredirect($data)) {
			$post = $this->parseForm($this->lib->cut_str($data, '<form name="F1"', '</form>'));
			$data = $this->lib->curl($url, $this->lib->cookie, $post);
			if(stristr($data,'>Error happened when generating Download Link.<'))   $this->error("Error happened when generating Download Link", true, false);
