...
class dl_depfile_us extends Download
{
    public function CheckAcc($cookie)
    {
        ...
        if (stristr($data, "premium'>")) {
            ...
            if (stristr($data2, "class='notice'>")) {
                return array(true, "Until " . $this->lib->cut_str($data, "premium'>", '<img') . "<br />" . $this->lib->cut_str($data2, "class='notice'>", "</p>"));
            }
    ...
    public function Leech($url)
    {
        ...
        } elseif (stristr($data, '<th>Download:</th>')) {
            $t = $this->lib->cut_str($data, "<th>Download:</th>", "</td>");

            if (preg_match("/<a href='(.*?)'>/", $t, $match)) {
                return trim($match[1]);
            }
