...
class dl_filespace_com extends Download {
    ...
    public function CheckAcc($cookie){
		...
		if(stristr($data, 'Premium account expire')) return array(true, "Until ".$this->lib->cut_str($data, '<TD><b>', '</b>'));
        else if(stristr($data, 'Account status') && !stristr($data, 'Premium account expire')) return array(false, "accfree");
    ...
    public function Leech($url) {
		...
		elseif(!$this->isredirect($data)){
			...
			elseif(stristr($page,"bold;\">Password:<")) $this->error("reportpass", true, false);
...
* Vinaget by ..::[H]::..
* Version: 2.7.0
* FileSpace.Com Download Plugin by rayyan2005[21.06.2015]
