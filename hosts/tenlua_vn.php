...
class dl_tenlua_vn extends Download {
    ...
    public function CheckAcc($cookie){
		...
		if ($data == "-500") return array(false, "accinvalid");
		else {
			$json = json_decode($data, true);
			if ($json[0]['utype'] <= 2) return array(false, "accfree");
			else return array(true, "Ngày hết hạn GOLD: ".$json[0]['endGold']);
		}
    }
  ...
  public function Login($user, $pass){
    ...
    public function Leech($url) {
		...
		if (isset($content[0]["reqlink"])) return false;
		elseif ($content[0]["type"]=="none") $this->error("dead", true, false, 2);
		elseif ($content[0]["type"]=="folder") $this->error("Not Support Folder Link", true, false, 2);
		else {
			$link = $content[0]["dlink"];
			$data = $this->lib->curl($link, "","");
			if ($this->isredirect($data)) return trim($this->redirect);
			else $this->error("Can Not Stream This Link", true, false, 2);
		}
		return false;
    }
