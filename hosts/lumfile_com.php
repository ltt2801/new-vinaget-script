...
class dl_lumfile_com extends Download
{
    ...
    public function CheckAcc($cookie)
    {
        ...
        if (stristr($data, 'Premium account expire:')) {
            return array(true, "Until " . $this->lib->cut_str($this->lib->cut_str($data, '<TR><TD align=right >Premium account expire:</TD>', '<TD><input type="button" value="Extend Premium Account"'), '<TD><b>', '</b></TD>'));
        } elseif (stristr($data, '<a href="/?op=payments">Upgrade Now</a>')) {
