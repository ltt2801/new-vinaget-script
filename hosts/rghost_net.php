...
class dl_rghost_net extends Download
{
    ...
    public function FreeLeech($url)
    {
        ...
        if ($pass) {
            ...
            if (stristr($data, 'Incorrect password')) {
        ...
        if (stristr($data, 'File is deleted.') || stristr($data, '<p>this page is not found</p>')) {
            $this->error("dead", true, false, 2);
        } elseif (stristr($data, 'password" name="password')) {
