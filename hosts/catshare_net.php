...
class dl_catshare_net extends Download
{
    public function CheckAcc($cookie)
    {
        ...
        if (stristr($data, 'Premium')) {
            return array(true, "Time left: " . str_replace("dni", "days", $this->lib->cut_str($data, 'remium</span>', '</a>')));
        }
    ...
    public function Leech($url)
    {
        ...
        if (stristr($data, "Nie ma takiej strony")) {
            $this->error("dead", true, false, 2);
        } elseif ($this->isredirect($data)) {
            return trim($this->redirect);
        } elseif (preg_match('/<form action="(.+)" method="GET">/', $data, $match)) {
            return trim($match[1]);
        }
